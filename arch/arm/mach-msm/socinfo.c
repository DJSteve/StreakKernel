...
static ssize_t
socinfo_show_raw_revision(struct sys_device *dev,
			  struct sysdev_attribute *attr,
			  char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", system_rev);
...
int check_two_g_three_g_mode(void)
{
	uint32_t two_g_three_g_mode = 0;
	uint32_t focus_key=0,capture_key=0;
	if(system_rev >= TOUCAN_EVT3_Band125 && system_rev < HWID_UNKNOWN)
	{
		focus_key = readl(GPIO_IN_1) & (1<< 17);
		capture_key = readl(GPIO_IN_2) & (1<< 8);
		if(focus_key != 0 && capture_key == 0) two_g_three_g_mode = 1;
	}
	else if(system_rev >= TOUCAN_EVT2_Band125 && system_rev <= TOUCAN_EVT2_Band148)
	{
		focus_key = readl(GPIO_IN_1) & (1<< 17);
		capture_key = readl(GPIO_IN_1) & (1<< 15);
		if(focus_key != 0 && capture_key == 0) two_g_three_g_mode = 1;
	}
	printk("focus_key=%x capture_key=%x two_g_three_g_mode=%x\n",focus_key,capture_key,two_g_three_g_mode);
	return two_g_three_g_mode;
}

static ssize_t
socinfo_show_two_g_three_g_mode(struct sys_device *dev,
			  struct sysdev_attribute *attr,
			  char *buf)
{
	uint32_t two_g_three_g_mode = check_two_g_three_g_mode();
	return snprintf(buf, PAGE_SIZE, "%u\n", two_g_three_g_mode);
}
#endif
...
static struct sysdev_attribute socinfo_v2_files[] = {
	...
	_SYSDEV_ATTR(raw_revision, 0444, socinfo_show_raw_revision, NULL),
#ifdef CONFIG_HW_TOUCAN
	_SYSDEV_ATTR(two_g_three_g_mode, 0444, socinfo_show_two_g_three_g_mode, NULL),
#endif
};
