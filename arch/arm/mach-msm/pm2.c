...
int msm_pm_wakeup_reason = 0;
EXPORT_SYMBOL(msm_pm_wakeup_reason);
	...
	(bool from_idle, uint32_t sleep_delay, uint32_t sleep_limit)
{
	...
	int collapsed = 0;
	int ret;
#if !defined(CONFIG_MSM_DEBUG_UART_NONE) && defined(CONFIG_DEBUG_LL)
	unsigned id;
#endif
...
#if defined(CONFIG_DEBUG_LL) && !defined(CONFIG_MSM_DEBUG_UART_NONE)
	id = P_UART1_CLK + CONFIG_MSM_DEBUG_UART -1;
...
#if defined(CONFIG_DEBUG_LL) && !defined(CONFIG_MSM_DEBUG_UART_NONE)
        id = P_UART1_CLK + CONFIG_MSM_DEBUG_UART -1;
        msm_proc_comm(PCOM_CLKCTL_RPC_ENABLE, &id, NULL);
#endif
...
{
	msm_rpcrouter_close();
	msm_proc_comm(PCOM_POWER_DOWN, &poweroff_reason, 0);
	for (;;)
		;
}
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
	...
	else if (code == SYS_POWER_OFF && _cmd) {
		char *cmd = _cmd;
		int len;
		struct timespec ts;
		struct rtc_time tm;
		...
		} else if (!strncmp(cmd, "Cover Removed", 14)) {
			poweroff_reason = 0x99887705;
		} else if (!strncmp(cmd, "Power Key", 10)) {
			poweroff_reason = 0x99887706;
		}
		...
		if (poweroff_reason != 0x998877BB) len += snprintf( log_buf + len, log_size-len, "PowerOff reason: %s(%x)\n", cmd, poweroff_reason);
		else len += snprintf( log_buf+len, log_size-len, "PowerOff reason: Unknown(%x)\n", poweroff_reason);

		if (!(len += dump_stack_to_buf(log_buf+len, log_size-len)))
			printk(KERN_INFO "%s: Copy kernel log failed\n", __func__);
		else {
			mm_segment_t oldfs;
			...
			filp = filp_open("/firstboot/poweroff.log", O_CREAT|O_RDWR|O_APPEND, S_IRWXU);
                        #else
                        filp = filp_open("/data/poweroff.log", O_CREAT|O_RDWR|O_APPEND, S_IRWXU);
                        #endif
			...
			if ( IS_ERR(filp) ) {
				...
				ret = vfs_write(filp, log_buf, len, &offset);
