...
#define DEBUG_RPC_TIMEOUT
#define DEBUG_RPC_TIMEOUT_WORKAROUND
...
#ifdef DEBUG_RPC_TIMEOUT_WORKAROUND
static struct delayed_work      rpc_zero_delayed_work;
static struct workqueue_struct *rpc_zero_wqueue;

static struct rpcrouter_xprt_info *l_xprt_info;
#endif

#ifdef DEBUG_RPC_TIMEOUT
void rpc_zdelay_work_func(struct work_struct *work)
{
    printk("%s => %d\n", __func__, l_xprt_info->xprt->read_avail());
}
EXPORT_SYMBOL(rpc_zdelay_work_func);
...
#ifdef DEBUG_RPC_TIMEOUT
        if (xprt_info->xprt->read_avail() == 0)
        {
            l_xprt_info = xprt_info;

#ifdef DEBUG_RPC_TIMEOUT_LOG
            printk("%s we got 0 packet, queue delayed work\n", __func__);
#endif
#ifdef DEBUG_RPC_TIMEOUT_WORKAROUND
            queue_delayed_work(rpc_zero_wqueue, &rpc_zero_delayed_work, 
			    msecs_to_jiffies(500));
#endif
        }
        else
        {
            if (l_xprt_info == xprt_info)
            {
#ifdef DEBUG_RPC_TIMEOUT_LOG
            printk("%s we got packet OK \n", __func__);
#endif
#ifdef DEBUG_RPC_TIMEOUT_WORKAROUND
            cancel_delayed_work_sync(&rpc_zero_delayed_work);
#endif
            }
        }
#endif
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
	printk("[RR]- ver=%d type=%d src=%d:%08x crx=%d siz=%d dst=%d:%08x\n",
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
		printk("[IO]CALL to %08x:%d @ %d:%08x (%d bytes)\n",
		   be32_to_cpu(rq->prog), be32_to_cpu(rq->vers),
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
		printk("[IO]REPLY to xid=%d @ %d:%08x (%d bytes)\n",
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
	printk("[IO]Writing %d bytes, max pkt size is %d\n",
	   tx_cnt, max_tx);
#endif
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
			printk("[IO]Wrote %d bytes First %d, Last 0 mid %d\n",
			   rc, first_pkt, mid);
#endif
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
			printk("[IO]Wrote %d bytes First %d Last 1 mid %d\n",
			   rc, first_pkt, mid);
#endif
...
#ifdef DEBUG_RPC_TIMEOUT_LOG
	printk("[IO]READ on ept %p (%d bytes)\n", ept, rc);
#endif
...
#ifdef DEBUG_RPC_TIMEOUT_WORKAROUND
    INIT_DELAYED_WORK(&rpc_zero_delayed_work, rpc_zdelay_work_func);
    rpc_zero_wqueue = create_singlethread_workqueue("rpc_zeroq");
    if(!rpc_zero_wqueue) 
    {
        printk("rpc_zero_wqueue create FAIL!\n");
    }
#endif
