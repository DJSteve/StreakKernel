...
#define HW_DEFAULT_MAX_VOLUME	15000
...
static int audio_tx_mute(struct audio_client *ac, uint32_t dev_id, int mute)
{
	...
	if ( rpc.device_id  == ADSP_AUDIO_DEVICE_ID_VOICE)
		rpc.path = ADSP_PATH_TX_WITH_CNG_DISABLE;
	else
	rpc.path = ADSP_PATH_TX;
	rpc.mute = mute;

	printk("audio_tx_mute, mute = %d, rpc.mute = %d, rpc.path  = %d, rpc.device_id  = 0x%x",mute,  rpc.mute, rpc.path, rpc.device_id  );
	return audio_ioctl(ac, &rpc, sizeof(rpc));
}
...
static void audio_rx_analog_enable(int en)
{
	...
	case ADSP_AUDIO_DEVICE_ID_NULL_SINK_FM_HEADSET:
	...
	case ADSP_AUDIO_DEVICE_ID_NULL_SINK_FM_SPEAKER:
		if (analog_ops->speaker_enable)
			analog_ops->speaker_enable(en);
		break;
...
#ifdef CONFIG_HW_AUSTIN
	if( (g_ac_5v_status ==1) && (!voice_started ) && ((acdb_id == CAD_HW_DEVICE_ID_SPKR_PHONE_STEREO) || (acdb_id == CAD_HW_DEVICE_ID_SPKR_PHONE_MONO))) 
	{
...
void q6audio_update_AC_5V_status(int ac_status)
{
	...
	mutex_lock(&audio_path_lock);
	if((g_ac_5v_status ==1) && (!voice_started ) &&
