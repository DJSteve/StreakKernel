...
static struct usb_composition usb_func_composition[] = {
    {
        ...
        .functions      = 0x12,
        .adb_product_id     = 0xB103,
        .adb_functions      = 0x12,
        .diag_product_id     = 0xB103,
        .diag_functions      = 0x6412
    },
	{
...
#ifdef CONFIG_USB_ANDROID_RNDIS
	{
		...
		.product_id         = 0xB101,
		.functions	    = 0xA,
		.adb_product_id     = 0xB101,
		.adb_functions	    = 0xA,
		.diag_product_id    = 0x9443,
		.diag_functions	    = 0x41A,
	},
	{
...
static struct msm_gpio qsd_spi_gpio_config_data[] = {
	...
	{ GPIO_CFG(21, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), "spi_pwr" },
...
static struct msm_gpio qsd_spi_gpio_config_data_evt2[] = {
	...
	{ GPIO_CFG(19, 1, GPIO_INPUT,  GPIO_NO_PULL, GPIO_2MA), "spi_miso" },
	{ GPIO_CFG(20, 1, GPIO_INPUT,  GPIO_NO_PULL, GPIO_2MA), "spi_cs0" },
	{ GPIO_CFG(32, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), "oledbl_en" },
...
static uint32_t camera_on_gpio_surf_table[] = {
    GPIO_CFG(104,  0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), 
    GPIO_CFG(105,  0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), 
};

static uint32_t camera_off_gpio_surf_table[] = {
	GPIO_CFG(104,  0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), 
	GPIO_CFG(105,  0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), 
};
...
extern int check_two_g_three_g_mode(void);
#endif
...
static void msm_hsusb_get_qctusb_driver(void)
{
	...
	if(check_two_g_three_g_mode())
	{
        printk("%s: enable usb debugging functions because of 2g3g mode\n", __func__);
        qct_usbdiag_debugging = 1;
	}
#endif

#if 0
    if (qct_usbdiag_debugging == 0)
    {
        usb_func_composition[0].functions = 0x12;
        usb_func_composition[0].adb_functions = 0x12;
    }
    else
    {
        usb_func_composition[0].functions = 0x6412;
        usb_func_composition[0].adb_functions = 0x6412;
    }
#endif
    ...
    {
        printk("%s: disable adb debugging functions %d\n", __func__, rpc_arg2);
        adb_debugging = 0;
    }
    else
    {
        printk("%s: enable adb debugging functions %d\n", __func__, rpc_arg2);
        adb_debugging = 1;
    }
#else
    adb_debugging = 1;
	...
	
	{
		...
		ProcArg1 = SMEM_PC_OEM_UART_LOG_STATUS;
	    	...
	    	if (ProcArg2Uart1)
	    	{
				...
				gpio_tlmm_config(GPIO_CFG(45, 1, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),GPIO_ENABLE);	
				gpio_tlmm_config(GPIO_CFG(46, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),GPIO_ENABLE);	
	    	}
	    	else
	    	{
	    			
				gpio_tlmm_config(GPIO_CFG(45, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),GPIO_ENABLE);	
				gpio_tlmm_config(GPIO_CFG(46, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),GPIO_ENABLE);	
	    	}
	}
