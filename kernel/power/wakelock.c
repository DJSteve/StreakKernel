...
static void watchdog_func(unsigned long data)
{
	...
	wake_lock_timeout(&watchdog_wake_lock, HZ*30); 
...
static void suspend(struct work_struct *work)
{
	...
	mod_timer(&suspend_watchdog_timer, jiffies + HZ*60);
	if (timer_pending(&watchdog_timer)) del_timer(&watchdog_timer);

	ret = pm_suspend(requested_suspend_state);

	mod_timer(&watchdog_timer, HZ*60*60);
...
#ifdef CONFIG_WAKELOCK_WATCHDOG
		if (lock == &main_wake_lock && timer_pending(&watchdog_timer)) {
...
#endif
#ifdef CONFIG_WAKELOCK_WATCHDOG
			if (has_lock && !timer_pending(&watchdog_timer)) {
