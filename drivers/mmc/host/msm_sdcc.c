...
static void msmsdcc_enable_cd_irq(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned long flags;
	...
	{
		spin_lock_irqsave(&host->status_irq_enable_lock, flags);
		if (atomic_read(&host->status_irq_enable) == 0)
		{
			printk(KERN_DEBUG "[SDCC] msmsdcc_enable_cd_irq::enable_irq\n");
			enable_irq(host->plat->status_irq);
		}
		atomic_set(&host->status_irq_enable, 1);
		spin_unlock_irqrestore(&host->status_irq_enable_lock, flags);
	}

	return;
}

static void msmsdcc_notify_card_present(struct mmc_host *mmc, unsigned int present)
{
    struct msmsdcc_host *host = mmc_priv(mmc);
    ...
    if (host->plat->status_irq)
    {
        spin_lock_irqsave(&host->irq_retry_lock, flags);
        if (present)
        {
            set_irq_type(host->plat->status_irq, IRQF_TRIGGER_HIGH);
            atomic_set(&host->irq_retry, 0);
            printk(KERN_DEBUG "[SDCC] set_irq_type::high\n");
        }
        else
        {
            if (atomic_read(&host->irq_retry) < 50)
            {
                set_irq_type(host->plat->status_irq, IRQF_TRIGGER_LOW);
                atomic_inc(&host->irq_retry);
                printk(KERN_DEBUG "[SDCC] set_irq_type::low\n");
            }
            else
            {
                set_irq_type(host->plat->status_irq, IRQF_TRIGGER_HIGH);
                atomic_set(&host->irq_retry, 0);
                printk(KERN_DEBUG "[SDCC] set_irq_type::high--retry fail\n");
            }
        }
        spin_unlock_irqrestore(&host->irq_retry_lock, flags);
    }
...
static int msmsdcc_get_card_detect(struct mmc_host *mmc)
{
    struct msmsdcc_host *host = mmc_priv(mmc);
    int ret = 1;
    ...
    if (host->plat->status_irq)
    {
        ret = gpio_get_value(host->plat->status_irq -64) ^ 1;
    }
...
static const struct mmc_host_ops msmsdcc_ops = {
	...
	.get_cd = msmsdcc_get_card_detect,
	
	
	
	.notify_card_present = msmsdcc_notify_card_present,
	.enable_cd_irq = msmsdcc_enable_cd_irq,
...
msmsdcc_check_status(unsigned long data)
{
	...
	if (!host->plat->status) {
		...
		if ((host->pdev_id == 1) || (host->pdev_id == 3))
		{
...
msmsdcc_platform_status_irq(int irq, void *dev_id)
{
	...
	{
		spin_lock_irqsave(&host->status_irq_enable_lock, flags);
		if (atomic_read(&host->status_irq_enable) == 1)
		{
			printk(KERN_DEBUG "%s: disable_irq\n", __func__);
			disable_irq_nosync(host->plat->status_irq);
		}
		atomic_set(&host->status_irq_enable, 0);
		spin_unlock_irqrestore(&host->status_irq_enable_lock, flags);
	}
...
{
	struct mmc_host *mmc = dev_get_drvdata(dev);
	int card_detect_status = 0;

	printk(KERN_DEBUG "[SDCC] %s+\n", __func__);

	if (mmc)
		card_detect_status = msmsdcc_get_card_detect(mmc);
...
static ssize_t
set_detect(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	printk(KERN_DEBUG "[SDCC] %s+\n", __func__);
	printk(KERN_DEBUG "[SDCC] %s-\n", __func__);
...
static DEVICE_ATTR(detect, S_IRUGO | S_IWUSR,
		show_detect, set_detect);
static struct attribute *dev_detect_attrs[] = {
	&dev_attr_detect.attr,
	NULL,
};
static struct attribute_group dev_detect_attr_grp = {
	.attrs = dev_detect_attrs,
};
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 1)
		PM_LOG_EVENT(PM_LOG_ON, PM_LOG_SDCARD_EXT);
	else if (pdev->id == 3)
		PM_LOG_EVENT(PM_LOG_ON, PM_LOG_SDCARD_INT);
	#else
	if (pdev->id == 3)
		PM_LOG_EVENT(PM_LOG_ON, PM_LOG_SDCARD_EXT);
	#endif
	...
	spin_lock_init(&host->status_irq_enable_lock);
	
	
	
	spin_lock_init(&host->irq_retry_lock);
	...
	ret = sysfs_create_group(&pdev->dev.kobj, &dev_detect_attr_grp);
	if (ret)
		goto sysfs_rm_grp;
...
static void
msmsdcc_shutdown(struct platform_device *dev)
{
...
#ifdef CONFIG_MMC_AUTO_SUSPEND
	if (test_and_set_bit(1, &host->suspended))
		return 0;
...
#ifdef CONFIG_MMC_AUTO_SUSPEND
	if (!test_and_clear_bit(1, &host->suspended))
		return 0;
...
static int
msmsdcc_suspend_auto(struct platform_device *dev, pm_message_t state)
{
	struct mmc_host *mmc = mmc_get_drvdata(dev);
	struct msmsdcc_host *host = mmc_priv(mmc);
	int rc = 0;
...
static int
msmsdcc_resume_auto(struct platform_device *dev)
{
	struct mmc_host *mmc = mmc_get_drvdata(dev);
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned long flags;
...
static int msmsdcc_auto_suspend(struct mmc_host *host, int suspend)
{
	...
	if (suspend)
		return msmsdcc_suspend_auto(pdev, PMSG_AUTO_SUSPEND);
	else
		return msmsdcc_resume_auto(pdev);
...
static int __init msmsdcc_init(void)
{
	...
	if (ret) {
		printk("BootLog, -%s-, Failed to create debug fs dir, ret=%d\n", __func__, ret);
		return ret;
	}
#endif
	printk("BootLog, -%s-\n", __func__);
