...
#define SET_RNDIS_STRIN "set_from_netd"
static u16 set_rndis;
static int android_set_rndis(const char *val, struct kernel_param *kp);
module_param_call(set_rndis, android_set_rndis, NULL,
					&set_rndis, 0664);
MODULE_PARM_DESC(product_id, "Setup rndis");
...
extern char qisda_imei_serialnum[17];
extern int qct_usbdiag_debugging;
...
#define MAIN_PID		0xb007
#define USB_TETHER_PID		0xb100
#endif
#ifdef CONFIG_HW_TOUCAN
#define MAIN_PID		0xb103
#define USB_TETHER_PID		0xb101
#endif
...
static int android_switch_composition(u16 pid)
{
	...
	if (qct_usbdiag_debugging && (func->diag_product_id != 0))
	{
	        product_id = func->diag_product_id;
		dev->functions = func->diag_functions;

	}
	/* Honour adb users */
	else if (dev->adb_enabled) {
...
static int android_set_pid(const char *val, struct kernel_param *kp)
{
	...
	if ((tmp == USB_TETHER_PID) && (_android_dev->rndis_enabled ==0))
	{
		pr_err("switch rndis failed...\n");
		goto out;
	}
	if ((tmp == MAIN_PID) && (_android_dev->rndis_enabled ==1))
	{
		pr_err("switch main composition failed...rndis is still active...\n");
		goto out;
	}
...
static int android_set_rndis(const char *val, struct kernel_param *kp)
{
	...
	pr_info ("%s set rndis_enabled : %s , %d\n", __func__, val, sizeof(SET_RNDIS_STRIN));
	if (!strncmp(val, SET_RNDIS_STRIN, sizeof(SET_RNDIS_STRIN) - 1))
	{
		pr_info ("%s set rndis_enabled : %s OK\n", __func__, val);
		if (val[sizeof(SET_RNDIS_STRIN)] == '1')
		{
			pr_info ("%s set rndis_enabled = 1\n", __func__);
			_android_dev->rndis_enabled = 1;
		}
		else
		{
			pr_info ("%s set rndis_enabled = 0\n", __func__);
			_android_dev->rndis_enabled = 0;
		}
	}
out:
	return ret;
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	strings_dev[STRING_SERIAL_IDX].s = qisda_imei_serialnum;
	dev->pdata = pdata;
