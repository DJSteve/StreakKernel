...
extern int adb_debugging;
struct proc_dir_entry *adb_debug;
static int adb_debug_read_proc_debug(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	int i;
	*eof = 1;	
	i = scnprintf(page, PAGE_SIZE, "%d\n", adb_debugging);
	printk(KERN_ERR "adbnodeinfo i=%d, buf=%s\n",i,page);
	return i;
}
...
int adb_function_init(void)
{
	...
	adb_debug = proc_mkdir("adb_debug", NULL);
	if (adb_debug == NULL) {
		printk(KERN_ERR "Unable to create /proc/adb_debug\n");
		return -ENOMEM;
	}

	
	ent = create_proc_entry("mode", 0, adb_debug);
	if (ent == NULL) {
		printk(KERN_ERR "Unable to create /proc/adb_debug/mode entry\n");
		ret = -ENOMEM;
	}
	else
	{
		ent->read_proc = adb_debug_read_proc_debug;
		printk(KERN_ERR "create /proc/adb_debug/mode entry\n");
