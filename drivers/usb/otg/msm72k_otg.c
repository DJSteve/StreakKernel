...
#ifdef TOUCAN_DOCK_SUPPORT

#define USB_CONNECT_DELAY_MSEC  msecs_to_jiffies(200)
...
static void usb_idpin_work(struct work_struct *work);
static void usb_connect_work(struct work_struct *w);
static void msm_otg_start_peripheral(struct otg_transceiver *xceiv, int on);

static DECLARE_WORK(usbotg_worker, usb_idpin_work);

static struct workqueue_struct *usb_work_queue;

static void usb_idpin_work(struct work_struct *work)
{
    printk(KERN_INFO "%s, switch dock state\n", __func__);
    switch_set_state(&dock_switch, dock_state);
}

static void usb_connect_work(struct work_struct *w)
{
	struct msm_otg *dev = container_of(w, struct msm_otg, usbcon_worker.work);

	if (!is_host() || (dock_state == 2))
			...
			struct usb_gadget *gadget)
{
	...
	if (is_host())
    {
        ...
        dock_state = 2;
        queue_work(usb_work_queue, &usbotg_worker);
    }
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
        ...
        else
        {
            ...
            dock_state = 2;
            queue_work(usb_work_queue, &usbotg_worker);
            if ((otgsc & OTGSC_BSVIS) && (otgsc & OTGSC_BSVIE))
            {
			    msm_otg_start_peripheral(&dev->otg, is_b_sess_vld());
            }
        }
		...
		pr_info("(%x) VBUS - (%s)\n", otgsc, otgsc & OTGSC_BSV ? "ON" : "OFF");
        if ((dock_state == 2) || !(otgsc & OTGSC_BSV))
        {
		    msm_otg_start_peripheral(&dev->otg, is_b_sess_vld());
        }
        else
        {
            schedule_delayed_work(&dev->usbcon_worker, USB_CONNECT_DELAY_MSEC);
        }
	}
#else
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
    ...
    {
        printk("%s, cannot register dock_switch\n", __func__);
    }

	INIT_WORK(&usbotg_worker, usb_idpin_work);
	INIT_DELAYED_WORK(&dev->usbcon_worker, usb_connect_work);

	usb_work_queue = create_singlethread_workqueue("usb_dock_workqueu");
#endif
