...
static int misc_psensor_release(struct inode *inode_p, struct file *fp)
{
	...
	DBGPRINTK(KERN_DEBUG "[Jackie] misc_psensor_release+\n");
	DBGPRINTK(KERN_DEBUG "[Jackie] misc_psensor_release-\n");
	return 0;
}

void psensor_powerup(void)
{
	int ret = 0;
	...
	if(psensor_power_counter == 1)
	{
		...
		{
			ret = vreg_set_level(vreg_psensor, 2600);
			if(ret)
			 printk(KERN_ERR "%s: vreg_psensor set 2.6V Fail, ret=%d\n", __func__,ret);
		}
...
static int psensor_param_set(const char *val, struct kernel_param *kp)
{
	...
	if(psensor_param==0)
	{
		printk(KERN_ERR "[Jackie] do nothing...\n");
	}
	else  if(psensor_param==1)
	{
		printk(KERN_ERR  "[Jackie] psensor_param_set(), ps_sd_status=%d, ps_out_status=%d\n", gpio_get_value(PSENSOR_ENABLE_GPIO), gpio_get_value(PSENSOR_INT_GPIO));
	}
	...
	{
		printk(KERN_ERR "[Jackie] else........do nothing...\n");
	}
