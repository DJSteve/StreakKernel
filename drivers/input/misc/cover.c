...
#include "/root/streak/3/kernel-old/arch/arm/mach-msm/proc_comm.h" 
...
int cover_irq_setup(int cover_int_pin)
{
    ...
    rc = request_irq(MSM_GPIO_TO_INT(cover_int_pin), cover_irq_handler, 
                  IRQF_TRIGGER_RISING, COVER_DETECT_NAME, NULL);
    if (rc < 0)
    {
...
static int cover_init(void)
{
    ...
    gpio_tlmm_config(GPIO_CFG(cover_det_int_gpio, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
