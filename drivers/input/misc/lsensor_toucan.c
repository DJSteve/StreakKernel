...
static int lsensor_misc_ioctl(struct inode *inode_p, struct file *fp, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd)
  {
    ...
    case PSENSOR_IOC_ENABLE:
      ...
      {
        MSG2("%s: (%04d) PSENSOR_IOC_ENABLE", __func__,current->pid);
        lsensor_enable_onOff(PSENSOR_EN_AP,1);
      }
      break;
...
static int __init lsensor_init(void)
{
  ...
  gpio_tlmm_config(GPIO_CFG(ls_drv.intr_gpio, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
  ...
  else
  {
    ...
    gpio_tlmm_config(GPIO_CFG(LSENSOR_GPIO_INT_EVT2, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
  }
