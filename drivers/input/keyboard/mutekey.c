...
static int __init mutekey_probe(struct platform_device *pdev);
...
static int mutekey_setup_gpio( struct mutekey_t *g_mk )
{
    ...
    rc = gpio_tlmm_config( GPIO_CFG(g_mk->gpio_num, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA),GPIO_ENABLE );
    if( rc )
    {
...
static void mutekey_irqWorkHandler( struct work_struct *work )
{
	...
	if(!rt_task(current))
    {
		...
		{
			MY_INFO_PRINTK( 4, "TEST_INFO_LEVEL¡G" "fail to set rt pri...\n" );
		}
		else
		{
			MY_INFO_PRINTK( 4, "TEST_INFO_LEVEL¡G" "set rt pri...\n" );
		}
    }
...
static int __init mutekey_init(void)
{
	int rc = 0;
