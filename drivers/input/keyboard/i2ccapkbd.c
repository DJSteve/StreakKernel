 ...
 static int qi2ccapkybd_config_gpio(int kbd_irqpin)
{
  ...
  gpio_tlmm_config(GPIO_CFG(kbd_irqpin, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
...
static void capsensor_early_resume(struct early_suspend *h)
{
  ...
  if (atomic_read(&capsensor_is_in_suspend))
  {
	...
	{
		printk(KERN_ERR "## Jackie ## %d ms passed after Capkey IC reset\n", sleep_ms_time);
	}
...
static struct device_attribute qsd_led_button_led_attrs[] = {
  __ATTR(led0, 0666, qsd_led_button_led_show, qsd_led_button_led_store),
  __ATTR(led1, 0666, qsd_led_button_led_show, qsd_led_button_led_store),
  __ATTR(led2, 0666, qsd_led_button_led_show, qsd_led_button_led_store),
};
...
static void qi2ccapkybd_fetchkeys(struct work_struct *work)
{
  ...
  if(!cap_i2c_read(kbdcl, &temp_rd, 1))
  {
	...
	if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	{
		...
		printk(KERN_ERR "[Jackie]temp_rd.data=0x%x\n", temp_rd.data);
	}
#endif
...
#if GETSOMELOG_AFTER_SCREENOFF
	  	if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	  		printk(KERN_ERR "[Jackie]report KEY[%d] Release at first\n",kbd_data.lastkeycode);
#endif
      }
...
#if GETSOMELOG_AFTER_SCREENOFF
	  if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	  	printk(KERN_ERR "[Jackie]report KEY_BACK KEYPRESS\n");
#endif
    }
...
#if GETSOMELOG_AFTER_SCREENOFF
	  if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	  	printk(KERN_ERR "[Jackie]report KEY_HOME KEYPRESS\n");
#endif
    }
...
#if GETSOMELOG_AFTER_SCREENOFF
	  if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	  	printk(KERN_ERR "[Jackie]report KEY_MENU KEYPRESS\n");
#endif
    }
...
#if GETSOMELOG_AFTER_SCREENOFF
	  	if (atomic_read(&GetSomeLogAfterLaterResume) >0 )
	  		printk(KERN_ERR "[Jackie]report KEY[%d] Release\n",kbd_data.lastkeycode);
#endif
    }
  }
  else
...
static void qi2ccapkybd_capsensor_init(struct i2c_client *kbd)
{
  ...
  {
    if(dynamic_log_ebable)
    printk(KERN_ERR "[Jackie]EVT1 Cap-key value, system_rev=%d\n", system_rev);
  ...
  else if((system_rev == EVT2_Band125)||(system_rev == EVT2_Band18))
  {
    ...
    if(dynamic_log_ebable)
    printk(KERN_ERR "[Jackie]EVT2-1 Cap-key value, system_rev=%d\n", system_rev);
    ...
    if(!cap_i2c_write(kbd, &temp_write, 1))
      ...
      printk(KERN_ERR "[Jackie]Menu default assign 0x00A0\n");
    ...
    if(!cap_i2c_write(kbd, &temp_write, 1))
      ...
      printk(KERN_ERR "[Jackie]HOME default assign 0xA300");
    ...
    if(!cap_i2c_write(kbd, &temp_write, 1))
      ...
      printk(KERN_ERR "[Jackie]BACK default assign 0x009F\n");
  }
  ...
  else if(system_rev <= EVT3P2_Band18)
  {
    ...
    if(dynamic_log_ebable)
    	printk(KERN_ERR "[Jackie]EVT2-2~EVT3-2 Cap-key value, system_rev=%d\n", system_rev);

    temp_write.addr = 0x11;
    if ( arg2MENU>0xA0 || arg2MENU<0x94)
    {
      temp_write.data = 0x00A0;     
      printk(KERN_ERR "[Jackie]FA Menu out of range=0x%x, default assign 0x00A0\n",arg2MENU);
    }
    else
    {
      temp_write.data = (unsigned short)arg2MENU;   
      if(dynamic_log_ebable)
      printk(KERN_ERR "[Jackie]FA Read Menu=0x%x\n",temp_write.data);
    }
    if(!cap_i2c_write(kbd, &temp_write, 1))
      printk(KERN_ERR "write  failed. temp_write.data=0x%x\n",temp_write.data);
    ...
    {
      temp_write.data = 0xA300;     
      printk(KERN_ERR "[Jackie]FA HOME out of range=0x%x, default assign 0xA300\n",arg2HOME);
    }
    else
    {
      temp_write.data = (unsigned short)((arg2HOME)<<8);    
      if(dynamic_log_ebable)
      printk(KERN_ERR "[Jackie]FA Read Home=0x%x\n",temp_write.data);
    }
    if(!cap_i2c_write(kbd, &temp_write, 1))
      printk(KERN_ERR "write  failed. temp_write.data=0x%x\n",temp_write.data);
    ...
    {
      temp_write.data = 0x009F;     
      printk(KERN_ERR "[Jackie]FA BACK out of range=0x%x, default assign 0x009F\n",arg2BACK);
    }
    else
    {
      temp_write.data = (unsigned short)arg2BACK;   
      if(dynamic_log_ebable)
      printk(KERN_ERR "[Jackie]FA Read Back=0x%x\n",temp_write.data);
    }
    if(!cap_i2c_write(kbd, &temp_write, 1))
      printk(KERN_ERR "write  failed. temp_write.data=0x%x\n",temp_write.data);
  }
  ...
  else
  {
    ...
    if(dynamic_log_ebable)
    	printk(KERN_ERR "[Jackie]EVT3-3~... SO380000QFN Cap-key value, system_rev=%d\n", system_rev);
...
static int capsensor_param_set(const char *val, struct kernel_param *kp)
{
  ...
  {
    printk(KERN_ERR "[Jackie] sleep=1, GPI=1\n");
  ...
  {
    printk(KERN_ERR "[Jackie] sleep=0, GPI=1\n");
  ...
  {
    printk(KERN_ERR "[Jackie] sleep=1, GPI=0\n");
