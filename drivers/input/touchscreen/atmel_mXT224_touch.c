...
{                 
	PRINT_IN 
        ...
        if(unlikely(atomic_read(&psensor_approached)))
		{
			...
			MY_INFO_PRINTK( 1,"INFO_LEVEL:"" Ignore touch\n" );
		}	
		else
		{
			save_multi_touch_struct( x_coord,y_coord,touch_status,report_id );
...
static int touchpad_poweron_device(int on)
{
	...
	if(on)
    {
        ...
        if( rc < 0 )
        {
            MY_INFO_PRINTK( 2,"ERROR_LEVEL:""failed to set %s to level %dmv\n",
        ...
        if( rc < 0 )
        {
            MY_INFO_PRINTK(2,"ERROR_LEVEL:""failed to enable %s to %dmv\n",
							atmel_power.id, atmel_power.mv);
            goto exit_power_switch;
        }
	...
	else
	{
        ...
        {
            MY_INFO_PRINTK( 2,"ERROR_LEVEL:""failed to disable %s\n",atmel_power.id);
            rc = -1;
            goto exit_power_switch;
        }
...
static int touchpad_setup_gpio( int gpio_num, int gpio_rst )
{
    ...
    rc = gpio_tlmm_config( GPIO_CFG(gpio_num, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA),GPIO_ENABLE );
    if( rc )
    {
        MY_INFO_PRINTK( 2,"ERROR_LEVEL:""touchpad_setup_gpio_num: remote config gpio_num %d failed (rc=%d)\n", gpio_num , rc );
    ...
    rc = gpio_tlmm_config( GPIO_CFG(gpio_rst, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA),
                                        GPIO_ENABLE );
    if( rc )
    {
        MY_INFO_PRINTK( 2,"ERROR_LEVEL:""touchpad_setup_gpio: remote config gpio_rst %d failed (rc=%d)\n", gpio_rst, rc);
		touchpad_release_gpio( gpio_num, gpio_rst );
        PRINT_OUT
        return rc;
    }
...
static int touchpad_config_mXT224(struct touchpad_t *g_tp)
{
    ...
    if( result )
    {
		MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "unable to write T6 backup byte 0x%x = 0x%x\n",regbuf,value );
    	PRINT_OUT
    	return result;
    }
    ...
    if( result )
    {
    	MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "unable to software reset T6 object 0x%x = 0x%x\n",maxTouchCfg_T6_obj.obj_addr,value );
    	PRINT_OUT
    	return result;
    }    	
...
static int check_module_vendor(struct touchpad_t *g_tp)
{
	...
	else 
	{
		...
		{
			MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "Neither tpk panel nor cando panel is !!!\n" );
			PRINT_OUT
			return result;
		}
	}
...
static int touchpad_detect_mXT224(struct touchpad_t *g_tp)
{
    ...
    {
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "unable to read reg 0xh and value failed 0x%x, return %d\n",value[i],result );
        PRINT_OUT
        return result;
    }
    ...
    for ( obj_addr = OBJECT_TABLE_ELEMENT_1 ; obj_addr < OBJECT_TABLE_ELEMENT_1+OBJECT_TABLE_ELEMENT_SIZE*g_tp->id_info.num_obj_element ; obj_addr += OBJECT_TABLE_ELEMENT_SIZE )
    {
        ...
        {
			MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_detect_mXT224: failed to fille in  obj table , it's overflow\n" );
        }
	}
	PRINT_OUT
    return 0;
...
static int power_on_flow_another_way(struct touchpad_t *g_tp)
{
	...
	if( result )
    {
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to config mXT224 (result=%d)\n",result );
        PRINT_OUT
        return result;
    }
    ...
    if( result )
    {
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" " the CHG pin is not low.)\n" );
		MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to get gpio_num value %d (result=%d)\n",g_tp->gpio_num,result );
        PRINT_OUT
        return result;
    }
    ...
    for (;;)
	{
    	...
    	if( value[0] >= maxTouchCfg_T6_obj.reportid_ub && value[0] <= maxTouchCfg_T6_obj.reportid_lb )
    	{
        	...
        	else
        	{
    			...
    			if ( result )
    			{
    				MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "failed to calculate config crc : %d\n", result );
    				PRINT_OUT
    				return -EFAULT;
    			}
    			break;
    	...
    	else
    	{
    		if ( j > 10 )
    		{
    			MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "failed to read T6 message\n" );
    			PRINT_OUT
    			return -EFAULT;
    		}
    		j++;	
	...
	for (;;)
	{
    	...
    	if( value[0] >= maxTouchCfg_T6_obj.reportid_ub && value[0] <= maxTouchCfg_T6_obj.reportid_lb )
    	{
    		...
    		if( (value[1] & CFGERR_MASK) != 0x0 )
    		{
        		...
        		if ( result )
    			{
    				MY_INFO_PRINTK( 2,"ERROR_LEVEL:""failed to power on flow another way : 0x%x  \n", result );
    				PRINT_OUT
    				return result;
    			}	
				PRINT_OUT
	...
	switch(cmd)
    {
		...
		case ATMEL_TOUCH_IOCTL_SET_KEYARRAY:
            ...
            if( copy_from_user( (void *)pData,
                                ...
                                length) )
            {
				MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "copy SET_KEYARRAY from user failed!\n" );
                result = -EFAULT;
				return result;
            }
			else
			{
				...
				if (result)
				{
					MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "failed to write SET_KEYARRAY!\n" );
					result = -EFAULT;
					return result;
				}
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write ctrl : %x\n", value[0]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write xorigin : %x\n", value[1]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write yorigin : %x\n", value[2]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write xsize : %x\n", value[3]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write ysize : %x\n", value[4]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write akscfg : %x\n", value[5]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write blen : %x\n", value[6]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write chthr : %x\n", value[7]);
				MY_INFO_PRINTK( 1,"INFO_LEVEL:" "write tchdi : %x\n", value[8]);
            }
            break;
		case ATMEL_TOUCH_IOCTL_GET_KEYARRAY:
			...
			if (result)
			{
				MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "failed to read GET_KEYARRAY!\n" );
                result = -EFAULT;
				return result;
			}
			result = copy_to_user( (void *)arg, &value[0], 9 );
			if (result)
			{
				MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "copy GET_KEYARRAY to user failed!\n" );
                result = -EFAULT;
				return result;
			}
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read ctrl : %x\n", value[0]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read xorigin : %x\n", value[1]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read yorigin : %x\n", value[2]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read xsize : %x\n", value[3]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read ysize : %x\n", value[4]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read akscfg : %x\n", value[5]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read blen : %x\n", value[6]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read chthr : %x\n", value[7]);
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "read tchdi : %x\n", value[8]);
            break;
...
static void tp_resume(struct early_suspend *h)
{
    ...
    if( result )
    {
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to config gpio\n" );
    }    
                                    ...
                                    struct i2c_client *client )
{
    ...
    if ( rc )
    {
        MY_INFO_PRINTK( 2,"ERROR_LEVEL:""failed to register input device\\n");
        input_free_device( input_dev );
    }else {
        *input = input_dev;
    }
...
static int __devinit touchpad_probe(struct i2c_client *client, const struct i2c_device_id *id )
{
    ...
    if(result)
	{
    	MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to power on device\n" );
		touchpad_poweron_device(0); 
        kfree(g_tp->multi);
		kfree(g_tp);
        PRINT_OUT
    ...
    if( result )
	{
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to setup gpio\n" );
		touchpad_poweron_device(0); 
    ...
    if( result )
    {
        MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to detect\n" );
		touchpad_poweron_device(0); 
        touchpad_release_gpio( g_tp->gpio_num,g_tp->gpio_rst );
        kfree(g_tp->multi);
		kfree(g_tp);
    ...
    if( result )
    {
    	MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to calculate infoblock crc\n" );
		touchpad_poweron_device(0); 
    	touchpad_release_gpio( g_tp->gpio_num,g_tp->gpio_rst );
    	kfree(g_tp->multi);
		kfree(g_tp);
    ...
    if( result )
    {
    	MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to power_on_flow_one_way\n" );
		touchpad_poweron_device(0); 
    	touchpad_release_gpio( g_tp->gpio_num,g_tp->gpio_rst );
    	kfree(g_tp->multi);
		kfree(g_tp);
    ...
    if( result )
    {
    	MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to register input\n" );
		touchpad_poweron_device(0); 
    ...
    if( result )
    {
		MY_INFO_PRINTK( 2, "ERROR_LEVEL:" "touchpad_probe: failed to register keyarray input\n" );
		touchpad_poweron_device(0); 
