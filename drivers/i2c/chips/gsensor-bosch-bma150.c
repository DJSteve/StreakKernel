...
static int i2c_gsensor_read(struct i2c_client *client, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_read+, regaddr:0x%x, len:%d\n", regaddr, len);

	ret = i2c_transfer(client->adapter, msgs, 2);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_read-ret:%d\n", ret);
	return ret;
}
...
static int i2c_gsensor_write(struct i2c_client *client, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	...
	if (len > 31)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_write::len > 31!!\n");
		return -ENOMEM;
	}

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_write+regaddr:0x%x, len:%d\n", regaddr, len);
...
static void debug_monitor_work_func(struct work_struct *work)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] debug_monitor_work_func+\n");
...
static int gsensor_power_down(struct gsensor_driver_data_t *gsensor_driver_data)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_power_down+\n");
	...
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_power_down::i2c_gsensor_write fail!!-ret:%d\n", ret);
		return ret;
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_power_down-ret:%d\n", ret);
	return ret;
}
...
static int gsensor_power_up(struct gsensor_driver_data_t *gsensor_driver_data)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_power_up+\n");
	...
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_power_up::i2c_gsensor_write fail!!-ret:%d\n", ret);
		return ret;
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_power_up-ret:%d\n", ret);
	return ret;
}
...
static int gsensor_set_ctrl_0b(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_ctrl_0b+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_ctrl_0b::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_ctrl_0b::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_settings_0c(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_0c+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0c::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0c::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_0c::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_settings_0d(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_0d+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0d::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0d::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_0d::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_settings_0e(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_0e+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0e::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0e::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_0e::tx_data!=rx_data\n");
	} while(1);
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_0e-\n");
	return 0;
}
...
static int gsensor_set_settings_0f(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_0f+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0f::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_0f::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_0f::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_settings_10(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_10+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_10::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_10::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_10::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_settings_11(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_settings_11+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_11::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_settings_11::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_settings_11::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_ctrl_14(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_temp;
	uint8_t rx_temp;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_ctrl_14+\n");

	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_BANDWIDTH, &rx_data, sizeof(rx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_ctrl_14::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return ret;
	}
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_ctrl_14::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_temp == rx_temp)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_ctrl_14::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_ctrl_15(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	uint8_t tx_data;
	uint8_t rx_data;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_ctrl_15+\n");
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_ctrl_15::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_ctrl_15::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (tx_data == rx_data)
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_ctrl_15::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_set_eeprom_write(struct gsensor_driver_data_t *driver_data, int enable_eeprom_write)
{
	...
	do
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_eeprom_write::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_eeprom_write::i2c_gsensor_read fail!!-ret:%d\n", ret);
			return ret;
		}
		...
		if ((tx_data & BMA150_EE_W_MASK) == (rx_data & BMA150_EE_W_MASK))
			break;
		msleep(20);
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_set_eeprom_write::tx_data!=rx_data\n");
	} while(1);
...
static int gsensor_default_config(struct gsensor_driver_data_t *gsensor_driver_data)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_default_config+\n");

	ret = gsensor_power_up(gsensor_driver_data);
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_default_config::gsensor_power_up fail!!-ret:%d\n", ret);
		return ret;
	}
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_default_config::i2c read chip id fail!!-ret:%d\n", ret);
		return ret;
	}

	if ((chipid[0] == 0x00) || (chipid[1] == 0x00)) {
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_default_config::bma150 accelerometer not detected\n");
		return -ENODEV;
	}
	...
	if (ret != 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_default_config:: The sleep is interrupted by a signal\n");
	}
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_default_config::i2c read bandwidth fail!!-ret:%d\n", ret);
		return ret;
	}
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_default_config::i2c_transfer fail!!-ret:%d\n", ret);
		return ret;
	}

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_default_config-ret:%d\n", ret);
	return 0;
}
...
static int gsensor_calibrate_config(struct gsensor_driver_data_t *driver_data)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calibrate_config+\n");
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_calibrate_config::i2c_gsensor_write fail!!-ret:%d\n", ret);
		return ret;
	}
	else ret = 0;
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_calibrate_config::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return ret;
	}
	else ret = 0;
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calibrate_config-ret:%d\n", ret);
	return ret;
}
...
static void gsensor_get_offset(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_get_offset+\n");

	memset(rx_buf, 0, sizeof(rx_buf));
	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_OFFSET_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_get_offset::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return;
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_get_offset-offset[0]:0x%x, offset[1]:0x%x, offset[2]:0x%x\n", offset[0], offset[1], offset[2]);
	return;
}
...
static void gsensor_set_offset(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_offset+offset_x:0x%x, offset_y:0x%x, offset_z:0x%x\n", offset[0], offset[1], offset[2]);

	memset(rx_buf, 0, sizeof(rx_buf));
	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_OFFSET_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_offset::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return;
	}
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_offset::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return;
		}
		msleep(20);
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_offset-\n");
	return;
}
...
static void gsensor_set_offset_eeprom(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_offset_eeprom+offset_x:0x%x, offset_y:0x%x, offset_z:0x%x\n", offset[0], offset[1], offset[2]);

	memset(rx_buf, 0, sizeof(rx_buf));
	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_OFFSET_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_offset_eeprom::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return;
	}
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_set_offset_eeprom::i2c_gsensor_write fail!!-ret:%d\n", ret);
			return;
		}
		msleep(20);
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_offset_eeprom-\n");
	return;
}
                                             ...
                                             unsigned short *offset_x, unsigned short *offset_y, unsigned short *offset_z)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calc_new_offset+offset_x:%d, offset_y:%d, offset_z:%d\n", *offset_x, *offset_y, *offset_z);
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calc_new_offset-ret:%d, offset_x:%d, offset_y:%d, offset_z:%d\n", calibrated, *offset_x, *offset_y, *offset_z);
	return calibrated;
}
...
static int gsensor_read_accel_xyz(struct gsensor_driver_data_t *driver_data, struct orientation_accel_t *accel)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_read_accel_xyz+\n");

	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_ACC_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_read_accel_xyz::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return 0;
	}
	...
	GSENSOR_LOGI(KERN_INFO "[GSENSOR] gsensor_read_accel_xyz::x:%d, y:%d, z:%d\n", accel->x, accel->y, accel->z);
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_read_accel_xyz-\n");
	return 1;
}
...
static int gsensor_read_accel_avg(struct gsensor_driver_data_t *driver_data, int num_avg, struct orientation_accel_t *min, struct orientation_accel_t *max, struct orientation_accel_t *avg )
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_read_accel_avg+\n");
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_read_accel_avg-ret:%d\n", comres);
	return comres;
}
...
static int gsensor_verify_min_max(struct orientation_accel_t min, struct orientation_accel_t max, struct orientation_accel_t avg)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_verify_min_max+\n");
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_verify_min_max-ret:%d\n", ver_ok);
	return ver_ok;
}
...
static int gsensor_calibrate(struct gsensor_driver_data_t *driver_data, struct orientation_accel_t orientation, int *tries)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calibrate+\n");
	...
	if (ret != 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_calibrate:: The sleep is interrupted by a signal\n");
	}
	...
	do {
		...
		GSENSOR_LOGI(KERN_INFO "[GSENSOR] gsensor_calibrate::min-->x:%d, y:%d, z:%d\n", min.x, min.y, min.z);
		GSENSOR_LOGI(KERN_INFO "[GSENSOR] gsensor_calibrate::avg-->x:%d, y:%d, z:%d\n", avg.x, avg.y, avg.z);
		GSENSOR_LOGI(KERN_INFO "[GSENSOR] gsensor_calibrate::max-->x:%d, y:%d, z:%d\n", max.x, max.y, max.z);
	...
	*tries = ltries - *tries;
	...
	
	{
		...
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_calibrate fail::tries=%d, verify_fail_times=%d\n", *tries, verify_fail_times);
	}
	else
	{
		...
		GSENSOR_LOGI(KERN_INFO "[GSENSOR] gsensor_calibrate success\n");
	}
	

	gsensor_default_config(driver_data);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_calibrate-ret:%d\n", !need_calibration);
...
static int gsensor_engineer_mode_calibrate(struct gsensor_driver_data_t *driver_data, int calibrate)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_engineer_mode_calibrate+\n");

	if (calibrate >=6 || calibrate < 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_engineer_mode_calibrate calibrate value invalid-calibrate:%d\n", calibrate);
		return -EFAULT;
	}
	...
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_engineer_mode_calibrate::gsensor_calibrate first fail\n");
		return -EFAULT;
	}
...
{
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_engineer_mode_set_parameter+\n");
...
static int gsensor_engineer_mode_get_accel(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_accel_t *ioctl_accel)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_engineer_mode_read_accel+\n");
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_engineer_mode_read_accel::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return ret;
	}
...
static void gsensor_work_func(struct work_struct *work)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_work_func+\n");

	memset(rx_buf, 0, sizeof(rx_buf));
	ret = i2c_gsensor_read(driver_data->i2c_gsensor_client, BMA150_REG_ACC_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_work_func::i2c_gsensor_read fail!!-ret:%d\n", ret);
		return;
	}
	...
	if (ret != 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] gsensor_work_func:: The sleep is interrupted by a signal\n");
	}
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] gsensor_work_func::i2c_gsensor_write fail!!-ret:%d\n", ret);
		return;
	}
...
static irqreturn_t gsensor_irq_handler(int irq, void *dev_id)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_irq_handler+\n");
...
static int misc_gsensor_open(struct inode *inode_p, struct file *fp)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_open+\n");

	ret = down_interruptible(&sem_open);
	if (ret != 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] misc_gsensor_open:: The sleep is interrupted by a signal\n");
	}
	...
	if (0 == atomic_read(&driver_data->open_count))
	{
		...
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_open::request_irq fail!!\n");
			up(&sem_open);
			return ret;
		}

		GSENSOR_LOGI(KERN_INFO "[GSENSOR] misc_gsensor_open::driver_data->i2c_gsensor_client->irq:%d\n", driver_data->i2c_gsensor_client->irq);

		tx_data = BMA150_RESET_INT_MASK;
		ret = i2c_gsensor_write(driver_data->i2c_gsensor_client, BMA150_REG_RESET_INT, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_open::i2c_gsensor_write fail!!-ret:%d\n", ret);
			up(&sem_open);
			return ret;
		}
	}
...
static int misc_gsensor_release(struct inode *inode_p, struct file *fp)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_release+\n");

	if (down_interruptible(&sem_open) != 0)
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] misc_gsensor_release:: The sleep is interrupted by a signal\n");
	}
	...
	if (0 > atomic_read(&driver_data->open_count))
	{
		GSENSOR_LOGW(KERN_WARNING "[GSENSOR] misc_gsensor_release:: open_count=%d\n", atomic_read(&driver_data->open_count));
...
static long misc_gsensor_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::Not GSENSOR_IOC_MAGIC\n");
		return -ENOTTY;
	}
	...
	if (_IOC_DIR(cmd) & _IOC_READ)
	{
		...
		if (ret)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::access_ok check err\n");
			return -EFAULT;
		}
	}
	...
	switch (cmd)
	{
		case GSENSOR_IOC_READ:
			...
			{
				GSENSOR_LOGW(KERN_WARNING "[GSENSOR] misc_gsensor_ioctl:: The sleep is interrupted by a signal\n");
			}
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_READ:copy_to_user fail-\n");
				ret = -EFAULT;
			}
			break;
		...
		case GSENSOR_IOC_READ_DIRECT:
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_READ_DIRECT:gsensor_engineer_mode_get_accel fail-\n");
				ret = -EFAULT;
				break;
			}
			...
			if (copy_to_user((void __user*) arg, &local_gsensor_ioctl, _IOC_SIZE(cmd)))
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_READ_DIRECT:copy_to_user fail-\n");
				ret = -EFAULT;
			}
			break;
		...
		case GSENSOR_IOC_EM_SET_PARAMETER:
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_SET_PARAMETER:copy_from_user fail-\n");
				ret = -EFAULT;
				break;
			}
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_SET_PARAMETER:gsensor_engineer_mode_set_parameter fail-\n");
				ret = -EFAULT;
			}
			break;
		...
		case GSENSOR_IOC_EM_GET_ACCEL:
			...
			if (gsensor_engineer_mode_get_accel(&gsensor_driver_data, &local_gsensor_ioctl_accel) < 0)
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_GET_ACCEL:gsensor_engineer_mode_get_accel fail-\n");
				ret = -EFAULT;
				break;
			}
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_GET_ACCEL:copy_to_user fail-\n");
				ret = -EFAULT;
			}
			break;
		...
		case GSENSOR_IOC_EM_CALIBRATE:
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_CALIBRATE:copy_from_user fail-\n");
				ret = -EFAULT;
				break;
			}

			GSENSOR_LOGI(KERN_INFO "[GSENSOR] misc_gsensor_ioctl::calibrate:%d\n", calibrate);

			if (gsensor_engineer_mode_calibrate(&gsensor_driver_data, calibrate) < 0)
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_EM_CALIBRATE:gsensor_engineer_mode_calibrate fail-\n");
				ret = -EFAULT;
			}
			break;
		...
		case GSENSOR_IOC_SET_CALIB_PARM:
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM:copy_from_user fail-\n");
				ret = -EFAULT;
				break;
			}
			GSENSOR_LOGW(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM: offset@ x=%d, y=%d, z=%d\n", calibrate_data.offset_x, calibrate_data.offset_y, calibrate_data.offset_z);

			
			if (calibrate_data.offset_x == 0 && calibrate_data.offset_y == 0 && calibrate_data.offset_z == 0)
			{
				GSENSOR_LOGE(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM: offset_x,y,z=0 err-\n");
				ret = -EFAULT;
				break;
			}
			...
			{
				GSENSOR_LOGE(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM: offset_x,y,z > 1023 err-\n");
				ret = -EFAULT;
				break;
			}
		...
		case GSENSOR_IOC_SET_CALIB_PARM_EEPROM:
			...
			if (copy_from_user(&calibrate_data, (void __user*) arg, _IOC_SIZE(cmd)))
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM_EEPROM:copy_from_user fail-\n");
				ret = -EFAULT;
				break;
			}
			GSENSOR_LOGW(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM_EEPROM: offset@ x=%d, y=%d, z=%d\n", calibrate_data.offset_x, calibrate_data.offset_y, calibrate_data.offset_z);

			
			if (calibrate_data.offset_x == 0 && calibrate_data.offset_y == 0 && calibrate_data.offset_z == 0)
			{
				GSENSOR_LOGE(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM_EEPROM: offset_x,y,z=0 err-\n");
				ret = -EFAULT;
				break;
			}
			...
			if (calibrate_data.offset_x > 1023 || calibrate_data.offset_y > 1023 || calibrate_data.offset_z > 1023)
			{
				GSENSOR_LOGE(KERN_DEBUG "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_SET_CALIB_PARM_EEPROM: offset_x,y,z > 1023 err-\n");
				ret = -EFAULT;
				break;
			}
		...
		case GSENSOR_IOC_GET_CALIB_PARM:
			...
			{
				GSENSOR_LOGE(KERN_ERR "[GSENSOR] misc_gsensor_ioctl::GSENSOR_IOC_GET_CALIB_PARM:copy_to_user fail-\n");
				ret = -EFAULT;
			}
			break;
...
{
	int ret = 0;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_read+\n");
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_read-\n");
	return ret;
}
...
{
	int ret = 0;
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_write+\n");
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] misc_gsensor_write-\n");
	return ret;
}
...
static int i2c_gsensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_probe+id->name:%s\n", id->name);
	...
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_probe::gsensor_config fail-ret:%d\n", ret);
		return ret;
	}
	...
	if (ret)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_probe:: misc_register error-ret:%d\n",ret );
		return ret;
	}
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_probe-ret:%d\n", ret);
	return ret;
}
...
static int i2c_gsensor_remove(struct i2c_client *client)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_remove+\n");
	...
	if (ret < 0)
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_remove::gsensor_power_down fail\n");

	i2c_set_clientdata(client, NULL);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_remove-ret:%d\n", ret);
	return 0;
}
...
static int i2c_gsensor_suspend(struct i2c_client *client, pm_message_t state)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_suspend+\n");

	driver_data = i2c_get_clientdata(client);

	ret = gsensor_power_down(driver_data);
	if (ret < 0)
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_suspend::gsensor_power_down fail\n");

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_suspend-ret:%d\n", ret);
	return ret;
}
...
static int i2c_gsensor_resume(struct i2c_client *client)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_resume+\n");

	driver_data = i2c_get_clientdata(client);

	ret = gsensor_power_up(driver_data);
	if (ret < 0)
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] i2c_gsensor_resume::gsensor_power_up fail\n");

	mdelay(2);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_resume-ret:%d\n", ret);
	return ret;
}
...
static int i2c_gsensor_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_command+\n");
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] i2c_gsensor_command-ret:%d\n", ret);
	return ret;
}
...
{
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_exit+\n");
