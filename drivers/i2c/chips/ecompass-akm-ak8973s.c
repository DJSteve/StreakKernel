...
static int i2c_ecompass_read(struct i2c_client *client, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_read+, regaddr:0x%x, len:%d\n", regaddr, len);

	ret = i2c_transfer(client->adapter, msgs, 2);
	if (ret < 0)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_read::i2c_transfer failed-ret:%d\n", ret);
		return ret;
	}

	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_read-\n");
	return 0;
}
...
static int i2c_ecompass_write(struct i2c_client *client, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	...
	if (len > 31)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_write:: len > 31!!\n");
		return -ENOMEM;
	}

	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_write+regaddr:0x%x, len:%d\n", regaddr, len);
	...
	if (ret < 0)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_write::i2c_transfer failed-ret:%d\n", ret);
		return ret;
	}

	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_write-\n");
	return 0;
}
...
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] ecompass_reset_pin+\n");
...
static irqreturn_t ecompass_irq_handler(int irq, void *dev_id)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] ecompass_irq_handler+\n");
...
static void debug_monitor_work_func(struct work_struct *work)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] debug_monitor_work_func+\n");
...
static int misc_ecompass_open(struct inode *inode_p, struct file *fp)
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_open+\n");
...
static int misc_ecompass_release(struct inode *inode_p, struct file *fp)
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_release+\n");
...
static long misc_ecompass_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_ioctl+cmd_nr:%d\n", _IOC_NR(cmd));

	if (_IOC_TYPE(cmd) != ECOMPASS_IOC_MAGIC)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::Not ECOMPASS_IOC_MAGIC\n");
		return -ENOTTY;
	}
	...
	if (cmd == ECOMPASS_IOC_REG_INT)
	{
		...
		if (ret)
		{
			ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::access_ok check err\n");
			return -EFAULT;
		}
	}
	else
	{
		if (cmd != ECOMPASS_IOC_RESET)
		{
			...
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::copy_from_user fail\n");
				return -EFAULT;
			}
		...
		if (cmd == ECOMPASS_IOC_READ || cmd == ECOMPASS_IOC_WRITE)
		{
			...
			if (!kbuf)
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_read::kmalloc fail!!\n");
				return -ENOMEM;
			}
		}
	}
	...
	switch(cmd)
	{
		...
		case ECOMPASS_IOC_READ:
			...
			if (ret)
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::i2c_ecompass_read fail-ret:%d\n", ret);
				ret = -EIO;
			...
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::ECOMPASS_IOC_READ:copy_to_user fail-\n");
				ret = -EFAULT;
				goto misc_ecompass_ioctl_err_exit;
			}
			break;
		case ECOMPASS_IOC_WRITE:
			...
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::ECOMPASS_IOC_WRITE:copy_from_user fail-\n");
				ret = -EFAULT;
				goto misc_ecompass_ioctl_err_exit;
			}
			...
			if (ret)
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::i2c_ecompass_write fail-ret:%d\n", ret);
				ret = -EIO;
				goto misc_ecompass_ioctl_err_exit;
			}
			break;
		case ECOMPASS_IOC_REG_INT:
			...
			{
				ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::ECOMPASS_IOC_REG_INT:copy_from_user fail-\n");
				ret = -EFAULT;
				goto misc_ecompass_ioctl_err_exit;
			}
			ecompass_driver_data.user_pid = pid;
			ECOMPASS_LOGI(KERN_INFO "[ECOMPASS] misc_ecompass_ioctl::user_pid=%d\n", ecompass_driver_data.user_pid);
			break;
		default:
			ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] misc_ecompass_ioctl::default-\n");
			ret = -ENOTTY;
			break;
	}
...
misc_ecompass_ioctl_err_exit:
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_ioctl-\n");
	return ret;
}
...
static ssize_t misc_ecompass_read(struct file *fp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_read+\n");
...
static ssize_t misc_ecompass_write(struct file *fp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_write+fp:0x%x, buf:0x%x, count:%d, f_pos:%d\n", &fp, &buf, count, *f_pos);

	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] misc_ecompass_write-\n");
	return ret;
}
...
static void ecompass_intr_work_func(struct work_struct *work)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] ecompass_intr_work_func+\n");
...
static int i2c_ecompass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_probe+\n");
	...
	ECOMPASS_LOGI(KERN_INFO "[ECOMPASS] i2c_ecompass_probe::gpio_intr:%d, gpio_rst:%d\n", ecompass_driver_data_p->gpio_intr, ecompass_driver_data_p->gpio_reset);
	...
	if (!client->irq)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_probe::IRQ number is NULL\n");
		ret = -1;
	...
	if (ret)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_probe::request_irq fail!!\n");
	...
	if (ret)
	{
		ECOMPASS_LOGE(KERN_ERR "[ECOMPASS] i2c_ecompass_probe:: misc_register error-ret:%d\n",ret );
...
static int i2c_ecompass_remove(struct i2c_client *client)
{
	...
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_remove+\n");
...
static int i2c_ecompass_suspend(struct i2c_client *client, pm_message_t state)
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_suspend+\n");
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_suspend-\n");
	return 0;
}
...
static int i2c_ecompass_resume(struct i2c_client *client)
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_resume+\n");
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_resume-\n");
	return 0;
}
...
static int i2c_ecompass_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_command+\n");
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] i2c_ecompass_command-\n");
	return 0;
}
...
{
	ECOMPASS_LOGD(KERN_DEBUG "[ECOMPASS] ecompass_exit+\n");
