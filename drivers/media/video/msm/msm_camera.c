...
static int austin_sensor_probe_cnt = 0;
static int (*austin_sensor_preinit_func[MSM_MAX_CAMERA_SENSORS])(void *);
static struct msm_camera_sensor_info *austin_sensor_info[MSM_MAX_CAMERA_SENSORS];
...
static ssize_t sensor_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    if( NULL == buf || NULL == attr )
    {
        CSDBG("sensor_show: invalid args\n");
        return 0;
    }

    if( !strcmp(attr->attr.name, "sensor") )
    {
        if( 0 == austin_sid )
        {
            char name[] = "ov5642";
            return sprintf(buf, "%s", name);
        }else if( 1 == austin_sid ) {
            char name[] = "ov7690";
            return sprintf(buf, "%s", name);
        }else {
            return 0;
        ...
        return sprintf(buf, "%d", austin_cnt);
    }else {
        CSDBG("sensor_show: unknown attr name\n");
        return 0;
    }
}

static ssize_t sensor_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if( NULL == buf || NULL == attr || (count < 7) )
    {
        CSDBG("sensor_store: invalid args\n");
        return 0;
    }

    if( !strcmp(attr->attr.name, "sensor") )
    {
        CDBG("sensor_store: stored name is %s, count is %d\n", buf, count);
        for( i=0 ; i<ARRAY_SIZE(sensors); i++ )
        {
            if( 0 == memcmp(sensors[i], buf, count-1) )
            {
                CSDBG("sensor_store: sensor %s found\n", sensors[i]);
                austin_sid = i;
                break;
            }
        }
        ...
        CSDBG("sensor_store: stored status is %s\n", buf);
        if( !strcmp("switching", buf) ) {
            austin_cnt = 1;
            return count;
        }else {
            austin_cnt = 0;
        ...
        CSDBG("sensor_store: unknown attr name\n");
        return 0;
    }
}

static struct device_attribute msm_camera_attrs[] =
{
    __ATTR(sensor, S_IRUGO | S_IWUGO, sensor_show, sensor_store),
    __ATTR(status, S_IRUGO | S_IWUGO, sensor_show, sensor_store),
};
		...
		struct msm_vfe_cfg_cmd *cfgcmd)
{
	...
	case CMD_AXI_CFG_SNAP:
        ...
        if( axi_data.bufnum1 < 2 )
        {
            region[1] = region[0];
            axi_data.bufnum1 = 2;
        }
        if( axi_data.bufnum1 < 3 )
        {
            region[2] = region[0];
            axi_data.bufnum1 = 3;
        }
        ...
        if( axi_data.bufnum2 < 2 )
        {
            region[axi_data.bufnum1+1] = region[axi_data.bufnum1];
            axi_data.bufnum2 = 2;
        }
        if( axi_data.bufnum2 < 3 )
        {
            region[axi_data.bufnum1+2] = region[axi_data.bufnum1];
            axi_data.bufnum2 = 3;
        }
...
static int __msm_open(struct msm_sync *sync, const char *const apps_id)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->vfefn.vfe_init) {
            ...
            {
                int i;
                for(i=0; i<austin_sensor_probe_cnt; i++)
				{
                    austin_sensor_preinit_func[i](austin_sensor_info[i]);
                    if( strcmp(austin_sensor_info[i]->sensor_name, sync->sdata->sensor_name) )
					{
					    
					    CSDBG("%s: sensor %s power down\n", __func__, austin_sensor_info[i]->sensor_name);
					    gpio_set_value(austin_sensor_info[i]->sensor_pwd, 1);
					}
				}
            }
#endif
				...
				struct msm_sensor_ctrl *))
{
	...
	wake_lock_init(&sync->wake_lock_suspend, WAKE_LOCK_SUSPEND, "msm_camera"); 
    ...
    preinit_func = sync->sdata->sensor_preinit;
#if 0
    preinit_func(sync->sdata);
#endif
#endif
	...
	if (rc >= 0) {
        ...
        austin_sensor_preinit_func[austin_sensor_probe_cnt] = preinit_func;
        austin_sensor_info[austin_sensor_probe_cnt++] = sync->sdata;
#endif
	}
#if 0
			...
			struct msm_sensor_ctrl *))
{
	...
	if (!msm_class) {
        ...
        austin_dev = device_create(msm_class, NULL, 0, NULL, "%s", "austin");
        if( IS_ERR(austin_dev) )
        {
            rc = PTR_ERR(austin_dev);
            pr_err("msm_camera: device create drvdata fail: %d\n",
                rc);
            return rc;
        }

        for( i=0; i<sizeof(msm_camera_attrs)/sizeof(msm_camera_attrs[0]); i++)
        {
            rc = device_create_file(austin_dev, (struct device_attribute *)&msm_camera_attrs[i]);
            if( rc )
            {
                pr_err("msm_camera: device create file %s fail\n",
                          msm_camera_attrs[i].attr.name);
                return rc;
            }
        }
