...
static uint printIrqStatus = 0;
module_param(printIrqStatus, uint, 0644);
...
static inline void vfe_read_irq_status(struct vfe_irq_thread_msg *out)
{
    ...
    printed = out->camifStatus || printIrqStatus || (out->vfeIrqStatus & 0x301);
#else
    printed = out->camifStatus || printIrqStatus || (out->vfeIrqStatus & 0x001);
#endif
    if( printed )
    {
	    CSDBG("camifStatus  = 0x%x vfeIrqStatus=0x%x\n",
                 out->camifStatus, out->vfeIrqStatus );
    }
...
void vfe_start(struct vfe_cmd_start *in)
{
	...
	*/
	...
	ctrl->vfeImaskLocal.camifEofIrq  = TRUE;
	ctrl->vfeImaskLocal.camifErrorIrq = TRUE;
