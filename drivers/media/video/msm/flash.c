	...
	unsigned led_state)
{
	...
	static atomic_t boost_enable = ATOMIC_INIT(0);	
	vreg_flashlight = vreg_get(NULL, "boost");

	rc = vreg_set_level(vreg_flashlight, 5000);
	if (rc) {
		pr_err(KERN_ERR "%s: vreg set level failed (%d)\n",
		 __func__, rc);
		return -EIO;
	}
		
	if( (led_state == MSM_CAMERA_LED_OFF) && atomic_read(&boost_enable))
	{
		pr_err("%s vreg_disable",__func__);	
	      	rc = vreg_disable(vreg_flashlight);
	    	if (rc)
		{
			pr_err("%s vreg_flashlight disable Fail, ret=%d",__func__,rc);	
		}
		else
		{
			atomic_dec(&boost_enable);
		}	
	}
	else if ((led_state != MSM_CAMERA_LED_OFF) && !atomic_read(&boost_enable))
	{
		pr_err("%s vreg_enable",__func__);	
	      	rc = vreg_enable(vreg_flashlight);
	    	if (rc)
		{
			pr_err("%s vreg_flashlight enable Fail, ret=%d",__func__,rc);	
		}
		else	
		{
			atomic_inc(&boost_enable);
		}	
	}	
	switch (led_state) {
	case MSM_CAMERA_LED_OFF:
