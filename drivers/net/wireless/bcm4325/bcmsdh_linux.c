...
int bcmsdh_suspend(struct device *dev);
int bcmsdh_resume(struct device *dev);

EXPORT_SYMBOL(bcmsdh_probe);
EXPORT_SYMBOL(bcmsdh_remove);

EXPORT_SYMBOL(bcmsdh_suspend);
EXPORT_SYMBOL(bcmsdh_resume);
#else
...
int bcmsdh_suspend(struct device *dev)
{
	bcmsdh_hc_t *sdhc, *prev;
	for(sdhc = sdhcinfo, prev = NULL; sdhc; sdhc = sdhc->next) {
		if (sdhc->dev == (void*)dev) break;
		prev = sdhc;
	}
	if (!sdhc) return 0;

	drvinfo.suspend(sdhc->ch);
