...
#ifdef TEST_DWORDMODE_F2_LOOPBACK

#define FR_LEN  (16 - 8)
#define HW_SW_HDR 8
#define TOT_LEN         (FR_LEN + HW_SW_HDR)

uint8 tx_frame[4096], rx_frame[4096];

void
test_f2_loopback(dhd_bus_t *bus, void *sdh)
{
	uint32 ii;
	uint16 totlen;
	int len;
	...
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xbf98, 4, 0x30000);
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xbf98, 4, 0x30001);
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xbf98, 4, 0x10000);

	
	dhdsdio_download_state(bus, FALSE);

	
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xaf98, 4, 0x30000);
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xaf98, 4, 0x30001);
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0xaf98, 4, 0x10000);

	
	OSL_DELAY(200000);
	
	dstatus = bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);
	if (STATUS_F2_RX_READY & dstatus)
		printf("Device F2 Rx is Ready\n");
	else
		printf("Device F2 Rx is NOT Ready\n");

	
	if (dhdsdio_set_siaddr_window(bus, SI_ENUM_BASE + 0x00010000)) {
		DHD_ERROR(("%s: FAILED to return to 0x18010000\n", __FUNCTION__));
		return;
	}
	
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0x00010000 + 0x9100, 4, 0x01000000);
	
	bcmsdh_reg_write(sdh, SI_ENUM_BASE + 0x00010000 + 0x9024, 4, 0);
	...
	for (ii = 0; ii < 4; ii++) {
		...
		memset(tx_frame, 0xaa+ii, totlen);
		bzero(rx_frame, totlen);

		
		*(uint16*)tx_frame = htol16(totlen);
		*(((uint16*)tx_frame) + 1) = htol16(~totlen);

		
		swheader = ((SDPCM_TEST_CHANNEL << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK)
			| bus->tx_seq;
		htol32_ua_store(swheader, tx_frame + SDPCM_FRAMETAG_LEN);

		ret = bcmsdh_send_buf(sdh, SI_ENUM_BASE, SDIO_FUNC_2, F2SYNC,
		                      tx_frame, totlen, NULL, NULL, NULL);

		OSL_DELAY(200000);
		
		dstatus = bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);

		DHD_ERROR(("Device status from regread = 0x%x\n", dstatus));
		DHD_ERROR(("Device status from bit-reconstruction = 0x%x\n",
		  bcmsdh_get_dstatus(bus->sdh)));

		if (dstatus & STATUS_F2_PKT_AVAILABLE) {
			len = ((dstatus & STATUS_F2_PKT_LEN_MASK) >> STATUS_F2_PKT_LEN_SHIFT)<<2;
			DHD_ERROR(("1 length to be read from gSPI = %d\n", len));
		} else {
			DHD_ERROR(("1 False pkt-available intr.\n"));
			len = 4;
		}

		
		ret = bcmsdh_recv_buf(sdh, SI_ENUM_BASE, SDIO_FUNC_2, F2SYNC,
		                      rx_frame, len, NULL, NULL, NULL);

		if (memcmp(rx_frame, tx_frame, totlen))
			printf("%s:Loopback failed.\n", __FUNCTION__);
		else
			printf("%s:Loopback passed.\n", __FUNCTION__);
	}
}
#endif 
...
#ifdef TEST_DWORDMODE_F2_LOOPBACK
	DHD_ERROR(("dstatus before f2 first loopback = 0x%x\n",
	           bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL)));
	test_f2_loopback(bus, bus->sdh);
	goto fail;
#endif 
