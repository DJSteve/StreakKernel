...
static struct flash_identification supported_flash[] =
{
	...
	{0x5551b3ad, 0xFFFFFFFF, (1024<<20),1, 2048, (2048<<6), 64, }, 	
    {0x5590bc2c, 0xFFFFFFFF, (512<<20), 1, 2048, (2048<<6), 64, }, 	
	...
	{0x5590ba98, 0xFFFFFFFF, (256<<20), 1, 2048, (2048<<6), 64, }, 
    {0x5590bc98, 0xFFFFFFFF, (512<<20), 1, 2048, (2048<<6), 64, }, 
...
msm_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (err) {
		...
		msm_nand_block_markbad(mtd, instr->addr);		
	} else {
...
static int __init msm_nand_init(void)
{
	int ret;
    printk("BootLog, +%s\n", __func__);
    ret = platform_driver_register(&msm_nand_driver);
