...
static void auo_lcd_bkl_onOff(unsigned int onOff)
{
  ...
  if(!inited)
  {
    gpio_tlmm_config(GPIO_CFG(57, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
    inited = 1;
...
static void auo_lcd_bkl_mode_switch(int mode, int repeat)
{
  ...
  if(!auo_lcd_enable)   goto exit_cancel;
...
static struct device_attribute auo_lcd_bkl_ctrl_attrs[] = {
  __ATTR(ctrl,  0222, NULL, auo_lcd_bkl_ctrl_store),  
  __ATTR(mode,  0222, NULL, auo_auo_lcd_bkl_mode_store),  
  __ATTR(ctrl2, 0666, bkl_eng_mode2_show, bkl_eng_mode2_store), 
};
static struct led_classdev auo_lcd_bkl = {
  .name     = "lcd-backlight",
  .brightness = LED_HALF, 
...
static int __init auo_probe(struct platform_device *pdev)
{
  int ret, i;
...
static int __init auo_init(void)
{
	int ret;
