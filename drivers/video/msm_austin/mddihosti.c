...
static uint32 mddi_reg_read_retry_max = 0;
...
static void mddi_process_rev_packets(void)
{
    ...
    MSG("#<< Get Packet %3d >>#",rev_pkt_ptr->packet_type);
		...
		if (mddi_rev_user.waiting) {
			...
			if ((!mddi_reg_read_successful) &&
				...
				if (!mddi_reg_read_successful)
				{
					if(mddi_reg_read_value_ptr)
					{
						*mddi_reg_read_value_ptr = 0xBEEFBEEF;
						printk("*mddi_reg_read_value_ptr = 0xBEEFBEEF\n");
					}
				}
...
void mddi_reset_llist(void)
{
  unsigned long flags;
  mddi_host_cntl_type *pmhctl = &(mhctl[mddi_curr_host]);
  uint16 reg_read_idx;  

  printk("[MDDI]### Reset llist ###\n\n");

  spin_lock_irqsave(&mddi_host_spin_lock, flags);
  if(pmhctl->llist_info.reg_read_idx != UNASSIGNED_INDEX)
  {
    reg_read_idx = pmhctl->llist_info.reg_read_idx;
    pmhctl->llist_info.reg_read_idx = UNASSIGNED_INDEX;
    if (pmhctl->llist_notify[reg_read_idx].waiting)
      complete(&(pmhctl->llist_notify[reg_read_idx].done_comp));
    #if 0
    if (pmhctl->llist_notify[reg_read_idx].done_cb != NULL)
    {
      (*(pmhctl->llist_notify[reg_read_idx].done_cb)) ();
    }
    #endif
    pmhctl->llist_notify[reg_read_idx].next_idx = UNASSIGNED_INDEX;
    pmhctl->llist_notify[reg_read_idx].in_use = FALSE;
    pmhctl->llist_notify[reg_read_idx].waiting = FALSE;
    pmhctl->llist_notify[reg_read_idx].done_cb = NULL;
    pmhctl->rev_state = MDDI_REV_IDLE;
    mddi_in_processing = 0;
  }
  spin_unlock_irqrestore(&mddi_host_spin_lock, flags);
