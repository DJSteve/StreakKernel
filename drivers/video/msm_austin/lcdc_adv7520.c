 ...
 static int lcdc_hdmi_config_gpio(int hdmi_irqpin)
{
	...
	{
		gpio_tlmm_config(GPIO_CFG(gpio_num, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_4MA), GPIO_ENABLE);
	}
	
	gpio_tlmm_config(GPIO_CFG(135, 1, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_8MA), GPIO_ENABLE);
	...
	gpio_tlmm_config(GPIO_CFG(44, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
	gpio_set_value(44,0);

	gpio_tlmm_config(GPIO_CFG(35, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);

	if(system_rev <= EVT1_4G_4G) 
	{
		gpio_tlmm_config(GPIO_CFG(85, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
		gpio_set_value(85,1);
	}
	else 
	{
		gpio_tlmm_config(GPIO_CFG(48, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
		gpio_set_value(48,1);
	}
	...
	hdmi_chip_powerup();
#endif
	gpio_tlmm_config(GPIO_CFG(161, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
	gpio_set_value(161,0);
	...
	gpio_tlmm_config(GPIO_CFG(HDMI_CABLEIN_GPIO, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
	...
	gpio_tlmm_config(GPIO_CFG(hdmi_irqpin, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_ENABLE);
...
printk(KERN_ERR "\n[HDMI] HDCP_INT\n" );
	...
	if(cable_out)
	{
		...
		gpio_tlmm_config(GPIO_CFG(161, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
		gpio_set_value(161,0);
#endif
		...
		if(system_rev <= EVT1_4G_4G) 
		{
			gpio_tlmm_config(GPIO_CFG(85, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA), GPIO_ENABLE);
			gpio_set_value(85,0);
		}
		else 
		{
		...
		gpio_tlmm_config(GPIO_CFG(35, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
