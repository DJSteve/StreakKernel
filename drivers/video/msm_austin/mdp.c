...
uint32 mddi_in_processing = FALSE;
...
extern ktime_t mdp_dma_s_last_update_time;
struct timeval mdp_dma_s_timeval;
...
void mdp_pipe_kickoff(uint32 term, struct msm_fb_data_type *mfd)
{
	...
	if (term == MDP_PPP_TERM) {
		...
		mdp_dma_s_last_update_time = ktime_get_real();
...
irqreturn_t mdp_isr(int irq, void *ptr)
{
	...
	do {
		...
		if (mdp_interrupt & MDP_DMA_S_DONE) {
			...
			mdp_dma_s_last_update_time.tv.sec =
				now_k.tv.sec - mdp_dma_s_last_update_time.tv.sec;
			mdp_dma_s_last_update_time.tv.nsec = 
				now_k.tv.nsec - mdp_dma_s_last_update_time.tv.nsec;

			if (mdp_debug[MDP_DMA_S_BLOCK]) {
				jiffies_to_timeval(jiffies, &now);
				mdp_dma_s_timeval.tv_usec =
					now.tv_usec - mdp_dma_s_timeval.tv_usec;
			}
...
static int mdp_probe(struct platform_device *pdev)
{
	...
	case EBI2_PANEL:
		...
		if (mfd->panel_info.pdest == DISPLAY_1) {
			...
			printk(KERN_ERR "[Jackie] mdp_probe: [DMA_S] add mdp_lut_update_nonlcdc here!\n");
			mfd->lut_update = mdp_lut_update_nonlcdc;
			mfd->do_histogram = mdp_do_histogram;
		}
#endif
