...
struct proc_dir_entry *uart_deubg;
unsigned int 			g_is_enable_deubg = 1;
...
static inline void wait_for_xmitr_done(struct uart_port *port, int bits)
{
...
static void msm_console_putchar(struct uart_port *port, int c)
{
	...
	wait_for_xmitr_done(port, UART_SR_TX_EMPTY);
...
static int uart_debug_read_proc_debug(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	*eof = 1;
	return sprintf(page, "uart_debug: %u\n", g_is_enable_deubg);
}
static int uart_debug_write_proc_debug(struct file *file, const char *buffer,
					unsigned long count, void *data)
{
	...
	if (enable == '0')
	{
		printk("Disable UART1 Ddebug\n");
		gpio_tlmm_config(GPIO_CFG(45, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA),GPIO_ENABLE);	
		gpio_tlmm_config(GPIO_CFG(46, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),GPIO_ENABLE);	
		g_is_enable_deubg = 0;
	}
	else
	{
		printk("Enable UART1 Ddebug\n");
...
static int __init msm_serial_init(void)
{
	...
	uart_deubg = proc_mkdir("uart_debug", NULL);
	if (uart_deubg == NULL) {
		pr_err("Unable to create /proc/uart_debug \n");
		return -ENOMEM;
	}

	
	ent = create_proc_entry("debug", 0, uart_deubg);
	if (ent == NULL) {
		pr_err("Unable to create /proc/uart_debug/debug entry");
		ret = -ENOMEM;
		goto fail;
	}
	ent->read_proc = uart_debug_read_proc_debug;
	ent->write_proc = uart_debug_write_proc_debug;
...
fail:
	...
	remove_proc_entry("debug", uart_deubg);
...
static void __exit msm_serial_exit(void)
{
	...
	remove_proc_entry("debug", uart_deubg);
	remove_proc_entry("uart_debug", 0);
